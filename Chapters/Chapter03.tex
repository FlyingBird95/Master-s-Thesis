...
This pricing model will then be used with the consumed resources to estimate the cost of running a container. According to \cite{bulla2014cloud} there exists different schemes and models for pricing for different service providers. However, the most common model employed in cloud computing is the ``pay-as-you go'' model. Customers pay a fixed price per unit of use. Amazon\footnote{\url{https://aws.amazon.com/}}, considered the market leader in cloud computing, utilizes such a model by charging a fixed price for each hour of virtual machine usage. 
...
p =  n_\text{CPU} * p_\text{CPU} + n_\text{GB} * p_\text{GB} + n_text{network} * p_\text{network}
\end{equation}
