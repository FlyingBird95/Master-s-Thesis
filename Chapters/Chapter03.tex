...
c =  n_\text{CPU} * p_\text{CPU} + n_\text{memory} * p_\text{memory} + n_\text{network} * p_\text{network}
\end{equation}

\noindent
Where $n_\text{CPU}$ is the amount of CPU requested on a specific host. $p_\text{CPU}$ is the price per CPU per hour. The same notion applies to $n_\text{GB}$ and $p_\text{GB}$. $n_\text{network}$ represents the amount of outgoing network in GB. Note that all cloud providers only charge for network traffic flowing out of their system, and not for communication between internal services. $p_\text{network}$ is the price of this outgoing network per GB.\\

\noindent
The pricing model $c$ can be categorized into a pricing model for used resources, and a pricing model for unused resources. This implies that $n_\text{used CPU} + n_\text{unused CPU} = n_\text{CPU}$. When it comes to the network cost, we decide that all external network are used, and therefore there is no network cost modelled in $c_\text{waste}$. 
